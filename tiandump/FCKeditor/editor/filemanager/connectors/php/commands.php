...
function DeleteFile( $resourceType, $currentFolder ) {
    ...
    if (is_file($file)) {
		...
		//echo '<error number="1" originaldescription="'.$file.'">' ;
    }
}
function DeleteFolder( $resourceType, $currentFolder ) {
	if ($resourceType == "File"){
		...
		$Folder_son = "image";
	}elseif ($resourceType == "Flash"){
		$Folder_son = "flash";
	}elseif ($resourceType == "Media"){
		$Folder_son = "media";
	}
	$folder = ServerMapFolder( $resourceType, $currentFolder, 'GetFolders' ) . $_GET['FolderName'];
//    $folder = $_SERVER['DOCUMENT_ROOT']."/FCKeditor_upload/" . $Folder_son . "/" . $_GET['FolderName'];
    if (is_dir($folder) )   {
        DELETE_RECURSIVE_DIRS($folder);
    } else {
        //echo '<error number="2" originaldescription="unable to locate folder">' ;
		  echo '<error number="2" originaldescription="'.$folder.'">' ;
    }
