...
require "fun/dm3.php";
$all_count = dm3('t', 0);
if($all_count_set != "" && $all_count_set != 0 && $all_count > $all_count_set){//當用戶設定讀取多少筆數據時
	$all_count = intval($all_count_set);
}
$totalpages = ceil($all_count / $pagerows);//總頁面
...
if ($pagemode == -1) {					// 不顯示
	...
	for ($i=$for_start; $i<$for_end; $i++) {
		$ti1 = $i + 1;
		$ti1 = ($curpage == $i) ? "<strong>".$ti1."</strong>" : "<a href='javascript:pag_change(".$i.",".$curpage.",".$totalpages.")'>".$ti1."</a>";
		echo $ti1;
		if($i+1 < $for_end){ echo " ‧ "; }
	}
